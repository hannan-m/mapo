...
public class MapoCodeFixProvider : CodeFixProvider
{
    ...
    public override async Task RegisterCodeFixesAsync(CodeFixContext context)
    {
        ...
        var configureMethod = classDeclaration.Members.OfType<MethodDeclarationSyntax>()
    ...
    private static async Task<Document> AddPartialModifierAsync(Document document, ClassDeclarationSyntax classDecl, CancellationToken cancellationToken)
    {
        var root = await document.GetSyntaxRootAsync(cancellationToken).ConfigureAwait(false);
        if (root == null) return document;

        var newClassDecl = classDecl.AddModifiers(SyntaxFactory.Token(SyntaxKind.PartialKeyword).WithTrailingTrivia(SyntaxFactory.Space));
        var newRoot = root.ReplaceNode(classDecl, newClassDecl);
        return document.WithSyntaxRoot(newRoot);
    }

    private async Task<Document> AddMappingAsync(Document document, MethodDeclarationSyntax configureMethod, string propertyName, CancellationToken cancellationToken)
    {
        var root = await document.GetSyntaxRootAsync(cancellationToken).ConfigureAwait(false);
        if (root == null) return document;
