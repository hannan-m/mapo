...
{
    public static (Dictionary<string, (string ParamName, ExpressionSyntax Body)> Maps, HashSet<string> Ignores, List<GlobalConverter> Converters, bool ShouldReverse)
        ParseSpecificConfiguration(List<MethodDeclarationSyntax> configMethods, ITypeSymbol source, ITypeSymbol target, SemanticModel model, List<Diagnostic> diagnostics = null)
    {
        ...
        {
            if (configMethod.ParameterList.Parameters.Count < 1) continue;
            var firstParamType = model.GetTypeInfo(configMethod.ParameterList.Parameters[0].Type!).Type as INamedTypeSymbol;
            if (firstParamType == null || firstParamType.TypeArguments.Length != 2) continue;

            var cfgSource = firstParamType.TypeArguments[0];
            var cfgTarget = firstParamType.TypeArguments[1];

            if (SymbolEqualityComparer.Default.Equals(cfgSource, source) &&
        ...
        foreach (var (configMethod, _) in matchedConfigs.OrderBy(c => c.IsExact))
        {
            ...
            foreach (var ig in cIgnores) ignores.Add(ig);
            converters.AddRange(cConverters);
            if (cReverse) shouldReverse = true;
        }
        ...
        ParseConfiguration(MethodDeclarationSyntax configMethod, SemanticModel model, List<Diagnostic> diagnostics = null)
    {
        ...
        {
            var configParamType = model.GetTypeInfo(configMethod.ParameterList.Parameters[0].Type).Type as INamedTypeSymbol;
        ...
        if (configMethod.Body != null)
            expressions.AddRange(configMethod.Body.Statements.OfType<ExpressionStatementSyntax>().Select(s => s.Expression));
                    ...
                    else if (ma.Name.Identifier.Text == "AddConverter" && inv.ArgumentList.Arguments.Count == 1)
                    {
                        ...
                        if (converterLambda != null)
                        {
                            ...
                            if (methodSymbol != null && methodSymbol.TypeArguments.Length == 2)
                            {
                                ...
                                converters.Add(new GlobalConverter(
    ...
    {
        if (lambda is SimpleLambdaExpressionSyntax s) return s.Parameter.Identifier.Text;
