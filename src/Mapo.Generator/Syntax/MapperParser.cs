...
public static class MapperParser
{
    ...
    public static ParseResult? Parse(SyntaxNode node, SemanticModel model, CancellationToken ct = default)
    {
        ...
        if (model.GetDeclaredSymbol(classDeclaration) is not INamedTypeSymbol classSymbol) return null;
        if (!classSymbol.GetAttributes().Any(a =>
            a.AttributeClass?.Name == MapperAttributeName &&
            a.AttributeClass.ContainingNamespace?.ToDisplayString() == MapperAttributeNamespace)) return null;
        ...
        var allConfigureMethodNodes = classDeclaration.Members.OfType<MethodDeclarationSyntax>()
            .Where(m => m.Identifier.Text == ConfigureMethodName).ToList();
        ...
        var mapperAttr = classSymbol.GetAttributes().FirstOrDefault(a =>
            a.AttributeClass?.Name == MapperAttributeName &&
        ...
        if (mapperAttr != null)
        {
            ...
            if (strictArg.Value.Value is bool b) strictMode = b;

            var trackingArg = mapperAttr.NamedArguments.FirstOrDefault(x => x.Key == "UseReferenceTracking");
            if (trackingArg.Value.Value is bool t) useReferenceTracking = t;
        }
        ...
        var partialMethodNodes = classDeclaration.Members.OfType<MethodDeclarationSyntax>()
            .Where(m => m.Modifiers.Any(mod => mod.ValueText == "partial")).ToList();
        ...
        foreach (var method in partialMethodNodes)
        {
            ...
            if (targetType is not INamedTypeSymbol) continue;
        ...
        while (discoveryQueue.Count > 0)
        {
            ...
            if (!processedPairs.Add((sourceType, targetType))) continue;
            ...
            if (partialSymbol != null && partialSymbol.Parameters.Length == 1 && !SymbolEqualityComparer.Default.Equals(partialSymbol.ReturnType, targetType))
            {
                partialSymbol = null;
            }

            bool isUpdateMapping = partialSymbol != null && partialSymbol.ReturnsVoid && partialSymbol.Parameters.Length == 2;
            var sourceName = partialSymbol?.Parameters[0].Name ?? "src";

            var (customMappings, ignoredProps, _, shouldReverse) = ConfigParser.ParseSpecificConfiguration(configureMethodNodes, sourceType, targetType, model, diagnostics);

            var discoveryList = new List<(ITypeSymbol, ITypeSymbol)>();
            var mapping = MethodMappingFactory.CreateMethodMapping(
                partialSymbol,
                sourceType, targetType, sourceName, customMappings, ignoredProps,
                partialMethodSymbols, enumMappingsToGenerate, injectedRenames, globalConverters, isUpdateMapping,
                partialSymbol?.DeclaringSyntaxReferences.FirstOrDefault()?.GetSyntax().GetLocation(),
                discoveryList, nameMap, model, isUserDeclared, diagnostics);

            // MAPO005: No accessible properties
            if (isUserDeclared && !mapping.IsEnumMapping && !mapping.IsCollectionMapping &&
                mapping.PropertyMappings.Count == 0 && mapping.ConstructorArgs.Count == 0 &&
                mapping.UnmappedProperties.Count == 0 && customMappings.Count == 0)
            {
                diagnostics.Add(Diagnostic.Create(
                    DiagnosticDescriptors.NoAccessibleProperties,
                    partialSymbol?.DeclaringSyntaxReferences.FirstOrDefault()?.GetSyntax().GetLocation() ?? Location.None,
            ...
            if (shouldReverse && !isUpdateMapping)
            {
                ...
                {
                    var reverseName = nameMap.TryGetValue(reversePair, out var rn) ? rn : ("Map" + TypeHelpers.CleanGenericName(targetType) + "To" + TypeHelpers.CleanGenericName(sourceType));
            ...
            foreach (var discovered in discoveryList)
            {
                ...
                {
                    var autoName = nameMap.TryGetValue(discovered, out var n) ? n : ("Map" + TypeHelpers.CleanGenericName(discovered.Item1) + "To" + TypeHelpers.CleanGenericName(discovered.Item2));
                    ...
                    discoveryQueue.Enqueue((discovered.Item1, discovered.Item2, autoName, false));
                         ...
                         !TypeHelpers.IsCollection(discovered.Item1) && !TypeHelpers.IsCollection(discovered.Item2))
                {
        ...
        {
            var sourceFields = entry.Value.Source.GetMembers().OfType<IFieldSymbol>().Where(f => f.HasConstantValue).ToList();
            var targetFields = entry.Value.Target.GetMembers().OfType<IFieldSymbol>().Where(f => f.HasConstantValue).ToList();

            var cases = new Dictionary<string, string>();
            foreach (var sField in sourceFields)
            {
                var tMatch = targetFields.FirstOrDefault(t => t.Name.Equals(sField.Name, System.StringComparison.OrdinalIgnoreCase));
                if (tMatch != null)
                {
                    cases[$"{entry.Value.Source.ToDisplayString()}.{sField.Name}"] = $"{entry.Value.Target.ToDisplayString()}.{tMatch.Name}";
                }
                else if (strictMode)
                {
                    diagnostics.Add(Diagnostic.Create(
            ...
            resultMappings.Add(new MethodMapping(
        ...
        var mapper = new MapperInfo(classSymbol.ContainingNamespace.ToDisplayString(), classSymbol.Name, isStatic, strictMode, useReferenceTracking, resultMappings, injectedMembers, globalConverters);
