...
{
    public static MethodMapping CreateMethodMapping(IMethodSymbol? methodSymbol, ITypeSymbol sourceType, ITypeSymbol targetType, string sourceName,
        Dictionary<string, (string ParamName, ExpressionSyntax Body)> customMappings, HashSet<string> ignoredProps,
        List<IMethodSymbol> partialMethods, Dictionary<string, (ITypeSymbol, ITypeSymbol)> enumMappings,
        Dictionary<string, string> injectedRenames, List<GlobalConverter> globalConverters, bool isUpdate, Location? location,
        List<(ITypeSymbol, ITypeSymbol)> discoveryList, Dictionary<(ITypeSymbol, ITypeSymbol), string> nameMap, SemanticModel model, bool isUserDeclared,
        List<Diagnostic> diagnostics = null)
    {
        ...
        if (!isUpdate && targetNamed != null)
        {
            ...
            var bestCtor = targetNamed.Constructors
            ...
            if (bestCtor != null)
            {
                ...
                {
                    if (PropertyMatcher.TryMatchSource(param.Name, param.Type, sourceType, sourceName, customMappings, partialMethods, enumMappings, injectedRenames, globalConverters, out var expression, discoveryList, nameMap, true, model, out var navSegments, out var requiresNullGuard, out var ctorOrigin, out var ctorNullMismatch))
                    {
                        if (ctorNullMismatch)
                        {
                            diagnostics?.Add(Diagnostic.Create(
                                ...
                                param.Name,
                                sourceType.GetMembers().OfType<IPropertySymbol>().FirstOrDefault(p => p.Name.Equals(param.Name, StringComparison.OrdinalIgnoreCase))?.Type.ToDisplayString() ?? "?",
            ...
            {
                if (ignoredProps.Contains(targetProp.Name) || handledByCtor.Contains(targetProp.Name)) continue;
                if (PropertyMatcher.TryMatchSource(targetProp.Name, targetProp.Type, sourceType, sourceName, customMappings, partialMethods, enumMappings, injectedRenames, globalConverters, out var expression, discoveryList, nameMap, false, model, out var navSegments, out var requiresNullGuard, out var propOrigin, out var propNullMismatch))
                {
                    if (propNullMismatch)
                    {
                        var sourcePropSymbol = sourceType.GetMembers().OfType<IPropertySymbol>().FirstOrDefault(p => p.Name.Equals(targetProp.Name, StringComparison.OrdinalIgnoreCase));
                        diagnostics?.Add(Diagnostic.Create(
                            Diagnostics.DiagnosticDescriptors.NullableToNonNullableMapping,
                            location ?? Location.None,
                            targetProp.Name,
                    ...
                    propMappings.Add(new PropertyMapping(targetProp.Name, expression!, targetProp.Type.IsValueType, targetProp.Type.SpecialType == SpecialType.System_String, isInitOnly, isRequired, requiresNullGuard, navSegments, null, propOrigin));
                }
                else
                {
        ...
        else if (isUpdate && targetNamed != null)
        {
            ...
            foreach (var targetProp in targetProps)
            {
                if (ignoredProps.Contains(targetProp.Name)) continue;
                if (PropertyMatcher.TryMatchSource(targetProp.Name, targetProp.Type, sourceType, sourceName, customMappings, partialMethods, enumMappings, injectedRenames, globalConverters, out var expression, discoveryList, nameMap, false, model, out var navSegments, out var requiresNullGuard, out var propOrigin, out var propNullMismatch))
                {
                    if (propNullMismatch)
                    {
        ...
        if (methodSymbol != null) mName = methodSymbol.Name;
        else if (nameMap.TryGetValue(key, out var n)) mName = n;
        else mName = "Map" + TypeHelpers.CleanGenericName(sourceType) + "To" + TypeHelpers.CleanGenericName(targetType);

        var mParams = methodSymbol?.Parameters.Select(p => $"{p.Type.ToDisplayString()} {p.Name}").ToList() 
    ...
    private static bool IsRequired(IPropertySymbol prop)
    {
        try
        {
            ...
            if (propInfo?.GetValue(prop) is true) return true;
        }
        catch { }
