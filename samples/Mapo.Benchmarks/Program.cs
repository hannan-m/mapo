    ...
    public static AutoMapper.IMapper CreateMapper()
    {
        var config = new AutoMapper.MapperConfiguration(cfg =>
        {
            ...
            cfg.CreateMap<Order, OrderDto>()
                .ConstructUsing((s, ctx) => new OrderDto(
                    s.Id,
                    s.OrderNumber,
                    s.Status.ToString(),
...
public partial class MapperlyBenchmarkMapper
{
    ...
    [Riok.Mapperly.Abstractions.MapProperty(nameof(Product.UnitPrice), nameof(ProductDto.PriceDisplay), Use = nameof(FormatPrice))]
    [Riok.Mapperly.Abstractions.MapProperty(new[] { nameof(Product.Category), nameof(Category.Name) }, new[] { nameof(ProductDto.CategoryName) })]
    [Riok.Mapperly.Abstractions.MapProperty(new[] { nameof(Product.Category), nameof(Category.Parent), nameof(Category.Name) }, new[] { nameof(ProductDto.ParentCategoryName) })]
    public partial ProductDto MapProduct(Product product);
...
public static class MapsterSetup
{
    public static TypeAdapterConfig CreateConfig()
    {
        ...
        config.NewConfig<Product, ProductDto>()
            ...
            .Map(d => d.ParentCategoryName, s => s.Category != null && s.Category.Parent != null ? s.Category.Parent.Name : (string?)null);
...
public class MappingBenchmarks
{
    ...
    public void Setup()
    {
        ...
        var laptops = new Category { Id = 2, Name = "Laptops", Parent = electronics };
        var peripherals = new Category { Id = 3, Name = "Peripherals", Parent = electronics };

        var products = new[]
        {
            new Product { SKU = "LAP-001", Name = "MacBook Pro 16\"", UnitPrice = 2499.00m, Category = laptops },
            new Product { SKU = "LAP-002", Name = "ThinkPad X1 Carbon", UnitPrice = 1899.00m, Category = laptops },
            new Product { SKU = "PER-001", Name = "MX Master 3S", UnitPrice = 99.99m, Category = peripherals },
            new Product { SKU = "PER-002", Name = "Keychron K2", UnitPrice = 79.00m, Category = peripherals },
            new Product { SKU = "GEN-001", Name = "USB-C Hub", UnitPrice = 45.50m, Category = null },
        };

        var addresses = new[]
        {
            new Address { Street = "123 Main St", City = "Seattle", State = "WA", ZipCode = "98101" },
            new Address { Street = "456 Oak Ave", City = "Portland", State = "OR", ZipCode = "97201" },
            new Address { Street = "789 Pine Rd", City = "San Francisco", State = "CA", ZipCode = "94102" },
        };

        _orders = Enumerable.Range(1, Count).Select(i =>
        {
    ...
    public List<OrderDto> Manual()
        => ManualMapper.MapOrders(_orders);

    [Benchmark]
    public List<OrderDto> Mapo()
        => _mapoMapper.MapOrders(_orders);

    [Benchmark]
    public List<OrderDto> AutoMapper()
        => _autoMapper.Map<List<OrderDto>>(_orders);

    [Benchmark]
    public List<OrderDto> Mapperly()
        => _mapperlyMapper.MapOrders(_orders);

    [Benchmark]
    public List<OrderDto> Mapster()
        => _orders.Adapt<List<OrderDto>>(_mapsterConfig);
}
